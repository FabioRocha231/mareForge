...
//!
//! A carta desenha as zonas lado a lado (`WorldMap::chart_position`), não
...
use marvyr_domain_world::map::CHART_CELL;
...
fn chart_bounds(map: &WorldMap) -> (Vec2, Vec2) {
    ...
    for area in &map.features().areas {
        let center = Vec2::new(area.cell.0 as f32, area.cell.1 as f32) * CHART_CELL;
...
fn chart_uv(map: &WorldMap, bounds: (Vec2, Vec2), x: f32, y: f32) -> Vec2 {
    let (cx, cy) = map.chart_position(x, y);
...
fn toggle_chart(
    ...
    (time, mut shot_done): (Res<Time>, Local<bool>),
) {
